	...
	do {
		...
		case RES_O_UPDATE_PROTOCOL: {
			...
			if (num) {
				...
				for (ri = 0; ri < num; ri++) {
					subTlvP[ri].id = ri;
					lwm2m_data_encode_int(data->protocol_support[ri],
							subTlvP + ri);
				}
			} else {
...
static uint8_t prv_firmware_write(uint16_t instanceId, int numData,
	...
	do {
		...
		case RES_M_PACKAGE_URI:
			...
			memset(uri, 0, 256);
			strncat(uri, (char*) dataArray[i].value.asBuffer.buffer,dataArray[i].value.asBuffer.length);
			firmware_update_download(uri);
...
static uint8_t prv_firmware_execute(uint16_t instanceId, uint16_t resourceId,
...
lwm2m_object_t * get_object_firmware(void) {
	...
	if (NULL != firmwareObj) {
		 ...
		 *
		 */
		firmwareObj->instanceList = (lwm2m_list_t *) lwm2m_malloc(
				sizeof(lwm2m_list_t));
